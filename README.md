...
业界常见的降本方向包括：**按任务复杂度做模型/能力分层**、与 **提示缓存、批处理** 等组合使用；具体型号与定价以各**云厂商与聚合网关的官方定价页**为准，README **不列举易过期的型号清单**。

在虾片内的落地分三层（与路线图勾选一致）：

- **P0（叙事与产品对齐）**：省钱首先是 **看清钱花在哪**——会话、模型、任务与时间维度的成本与回放已具备基础。
- **P1（本地洞察与建议）**：基于历史日志的统计与**启发式建议**（例如高消耗任务、模型分布），**不代替用户自动改 Agent 配置**，除非另发版说明。
- **P2（可选，重依赖生态）**：与运行时/网关的深度联动（如自动路由），单独里程碑，需兼容与安全评审。

当前路线图主项仍为「智能路由省钱」下的 **P0 → P1 递进**；欢迎通过 issue 讨论你的场景。
