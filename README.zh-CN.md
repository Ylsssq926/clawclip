...
### 找出 token 在哪里被浪费
重试循环。Prompt 膨胀。输出冗长。拿贵模型干轻活。虾片找出悄悄拉高账单的模式——告诉你先改哪个最值。

### 并排对比模型和配置
用不同模型或 Prompt 跑同一个任务，直接对比：哪个分更高、哪个花得少、哪个真的有进步。换完不用再靠感觉猜。

### 证明一次优化到底有没有用
每次评测都会存。改完之后，你能看到前后证明：分数、token、成本。直接结论：变好了、变差了，还是没区别。分数涨了但账单涨更多，你也会看到。

### 逐步回放任何一次运行
看 Agent 实际做了什么：每次工具调用、重试、推理、回复，按顺序展开。找到出问题那一步，不用翻原始 JSONL。
