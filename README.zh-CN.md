...
**你的 Agent 在重试循环、膨胀的 Prompt 和拿贵模型干轻活上烧 token。虾片告诉你钱死在哪——让你砍掉浪费，留下真正让它变强的东西。**

> **OpenClaw 内建的 Usage Dashboard 告诉你花了多少。虾片告诉你为什么花这么多、改了有没有用、怎么少花。**
>
> 虾片补的是 OpenClaw 没有的：逐步回放、版本对比、跨框架分析（Hermes、LangGraph、AutoGen）、可执行的优化建议。
...
**你在跑 OpenClaw 或 ZeroClaw。Token 账单一直涨。你换模型、改 Prompt，然后祈祷有用——但你从来不确定。**

虾片让你不用再猜。它读你的会话日志，回放实际发生了什么，给出评分，精确展示钱花在哪。砍掉浪费。选配得上价格的模型。只留真正让 Agent 变强的改动。
...
### 找出 Agent 在哪里烧钱
你没注意到的重试循环。越写越长的 Prompt。拿贵模型干简单活。虾片读你的日志，标出模式，告诉你先改哪个最省钱。

### 对比运行，不用靠猜
换个模型，改个 Prompt，再跑一次。虾片给你前后对比：分数、token、成本。变好了但更贵了，你也会看到。

### 给解决方案，不只是问题
发现浪费模式？虾片推荐免费 tier 替代方案，直接生成配置。不用自己翻文档。

## 关于诊断报告

> 运行诊断是**启发式诊断**，不是排行榜。它读的是这次运行里已经出现的信号，比如工具使用模式、重试循环、安全提示和成本结构，帮你更快发现问题。用来追踪相对变化，不适合横向排名。
...
| 你真正想知道的 | 虾片怎么回答 |
| --- | --- |
| **token 预算都花到哪去了？** | **成本报告** 按模型、任务和会话拆解花费，带浪费信号和省钱建议 |
| **Agent 真的在变强吗？** | **运行诊断** 读取会话里的行为信号（工具使用模式、重试循环、成本结构），帮你更快发现退步。用来追踪相对变化，不适合横向排名。 |
