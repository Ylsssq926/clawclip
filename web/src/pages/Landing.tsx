...
const LANDING_LOCAL_COPY = {
  zh: {
    ...
    'landing.hero.title': '一次 Agent 运行结束后，答案会慢慢浮上来。',
    'landing.hero.kicker': '先回到现场，看关键步骤是怎么发生的；再用一句判断和一张账单，知道这次到底稳不稳、值不值。',
    'landing.hero.meta': 'Demo 先看一遍也很自然，接入你的本地日志后，节奏不变。',
    'landing.cta.demoPrimary': '看示例回放',
    'landing.hero.source': 'GitHub',
    'landing.hero.media.badge': '精选样本',
    'landing.hero.media.caption': '先看一段精选样本，回放、成绩单和成本会顺着同一条线慢慢接上。',
    'landing.hero.media.pill.replay': '证据链正在展开',
    'landing.hero.media.pill.score': '74 / 100',
    'landing.hero.media.pill.cost': '$3.42 · 重试先修',
    'landing.flow.eyebrow': '顺着一次运行往下看',
    'landing.flow.title': '证据、判断和花费，会自己接上。',
    'landing.flow.subtitle': '先把发生过的事看明白，取舍就不会飘。',
  ...
  en: {
    'landing.hero.eyebrow': 'Replay · Scorecard · Cost',
    'landing.hero.title': 'When an agent run settles, the answers rise to the surface.',
    'landing.hero.kicker': 'Start by replaying the run while it is still vivid, then let one judgment and one bill tell you whether it held — and whether it earned the spend.',
    'landing.hero.meta': 'Try the demo first, then swap in your local logs without changing the flow.',
    'landing.cta.demoPrimary': 'Watch demo replay',
    'landing.hero.source': 'GitHub',
    'landing.hero.media.badge': 'Featured sample',
    'landing.hero.media.caption': 'Start with a curated run where replay, scorecard, and cost move in the same rhythm.',
    'landing.hero.media.pill.replay': 'Trail unfolding',
    'landing.hero.media.pill.score': '74 / 100',
    'landing.hero.media.pill.cost': '$3.42 · fix retries first',
    'landing.flow.eyebrow': 'Follow one run',
    'landing.flow.title': 'Evidence, judgment, and cost stay in the same breath.',
    'landing.flow.subtitle': 'Once the run is clear, the tradeoff stops drifting.',
